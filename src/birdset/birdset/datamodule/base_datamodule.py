...
class BaseDataModuleHF(L.LightningDataModule):
    ...
    def _load_data(self, decode: bool = True) -> DatasetDict:
        ...
        cache_dir = self.dataset_config.cache_dir or self.dataset_config.data_dir
        dataset_args = {
        ...
        if os.path.exists(self.dataset_config.hf_path):
            dataset_args["data_dir"] = self.dataset_config.data_dir

        if self.dataset_config.trust_remote_code:
